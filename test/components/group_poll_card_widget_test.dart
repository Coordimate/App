...
void main() {
  ...
  group('GroupPollCard', () {
    testWidgets('should navigate to CreateGroupPollPage when tapped',
        (WidgetTester tester) async {
      const String testGroupId = 'testGroupId';
      when(mockGroupController.fetchPoll(testGroupId))
    ...
    testWidgets('should display active poll when available',
        (WidgetTester tester) async {
      const String testGroupId = 'testGroupId';
      final testPoll =
          GroupPoll(question: 'Test Poll', options: ['Option 1', 'Option 2']);
      when(mockGroupController.fetchPoll(testGroupId))
          .thenAnswer((_) async => testPoll);
      await tester.pumpWidget(MaterialApp(
          home: GroupPollCard(
              groupId: testGroupId,
              initialPoll: testPoll,
              fontSize: 16.0,
              isAdmin: false)));
    ...
    testWidgets('should dismiss poll and delete on swipe',
        (WidgetTester tester) async {
      const String testGroupId = 'testGroupId';
      final testPoll =
          GroupPoll(question: 'Test Poll', options: ['Option 1', 'Option 2']);
  ...
  group('CreateGroupPollPage', () {
    testWidgets('should display question and options fields',
        (WidgetTester tester) async {
      const String testGroupId = 'testGroupId';
      await tester.pumpWidget(
          const MaterialApp(home: CreateGroupPollPage(groupId: testGroupId)));

      expect(find.byType(QuestionTextField), findsOneWidget);
      expect(find.byType(AddOptionButton), findsOneWidget);
    });

    testWidgets('should add an option when AddOptionButton is tapped',
        (WidgetTester tester) async {
      const String testGroupId = 'testGroupId';
      await tester.pumpWidget(
          const MaterialApp(home: CreateGroupPollPage(groupId: testGroupId)));
    ...
    testWidgets('should call createPoll and pop when valid input is provided',
        (WidgetTester tester) async {
      const String testGroupId = 'testGroupId';
      await tester.pumpWidget(
          const MaterialApp(home: CreateGroupPollPage(groupId: testGroupId)));
  ...
  group('VoteGroupPollPage', () {
    ...
    testWidgets('should display poll question and options',
        (WidgetTester tester) async {
      const String testGroupId = 'testGroupId';
      await tester.pumpWidget(MaterialApp(
          home: VoteGroupPollPage(
              groupId: testGroupId,
              poll: poll,
              memberAvatars: const {
            '12345': Avatar(size: 30, userId: '12345')
          })));
    ...
    testWidgets('should place a vote and update vote count',
        (WidgetTester tester) async {
      AppState.authController.userId = '12345';
      const String testGroupId = 'testGroupId';
      await tester.pumpWidget(MaterialApp(
